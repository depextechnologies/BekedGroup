...
export function BlogArticle({ post }: { post: FullPost }) {
  ...
  const content = locale === 'fr' ? post.contentFr || post.contentEn : post.contentEn || post.contentFr;

  return (
    <article data-testid="blog-article" className="relative">
      {/* HERO */}
      <section className="relative pt-32 pb-12 md:pt-40 md:pb-16 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0 bg-gradient-to-br from-bg-primary via-bg-primary to-[#0c0c0c]" />
          <div className="absolute top-1/3 left-1/4 h-[360px] w-[360px] rounded-full bg-brand-gold/12 blur-[140px]" />
        </div>
        ...
        <div className="relative max-w-4xl mx-auto px-6 md:px-12">
          ...
          >
            ...
            <div className="flex items-center gap-3 text-xs uppercase tracking-wider mb-5">
              <span className="inline-flex items-center px-3 py-1.5 rounded-full bg-brand-gold text-bg-primary font-bold">
                {post.category}
              </span>
...
function ShareButton({ title }: { title: string }) {
  ...
  return (
    <button
      onClick={onShare}
      data-testid="blog-article-share"
