...
- **2026-02-24 — Premium Support Center + Partner pipeline (P1):** Built `/support` with 8 sections (Hero with live-filter search, 12-card Categories grid, 12-item FAQ accordion with search filtering + empty state, 7-card "Info to provide" checklist, Customer Support form, Partner Application form, Commitment 4-stat panel, App Download with floating-mockup iPhone). Bilingual FR/EN, premium Framer Motion. Header nav got `Assistance & Support` / `Support Center` link between Advertising and Contact. Two new Prisma models: `SupportTicket` (open→in_progress→resolved→closed) and `PartnerApplication` (new→reviewing→approved→rejected). Public `POST /api/support/ticket` (Zod-validated, routes to customerservice@baked.group via nodemailer) and `POST /api/support/partner` (routes to partners@baked.group). Admin endpoints `GET /api/admin/support/{tickets,partners}` and `PATCH|DELETE /api/admin/support/{tickets,partners}/[id]` (P2025→404). New admin pages `/admin/support/tickets` and `/admin/support/partners` share a reusable `LeadsManager` component (list w/ unread golden ring + selectable detail card + status pills + mailto/tel deep-links + delete). AdminShell sidebar gained Support (LifeBuoy) + Partners (Handshake) entries. Sitemap updated with `/support`. SEO: title, og:tags, FAQPage JSON-LD. **Tested by testing_agent_v3_fork (iteration_23.json) — 100% PASS** (9/9 backend endpoints + 19/19 frontend checks: 8 sections, 12 categories, 12 FAQ items, 7 checklist, hero search live-filter incl. empty state, accordion toggle, locale switch, both forms POST + success state, admin status PATCH + delete, all regressions clean).
